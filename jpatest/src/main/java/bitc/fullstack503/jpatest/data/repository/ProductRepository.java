package bitc.fullstack503.jpatest.data.repository;
...
//  JpaRepository 를 상속받았기 때문에 JpaRepository 가 제공하는 기본 메소드와 쿼리 메소드를 생성하여 사용할 수 있음

//  쿼리메소드 : JpaRepository에서 제공하는 기본 메소드를 가지고 원하는 모든 기능을 사용할 수 없기 때문에 특정 키워드를 사용하여 사용자 정의 메소드를 생성하는 방식의 메소드

//  쿼리 메소드 생성 방법
//  리턴타입 + {주제 키워드 + 서술어} 를 조합하여 메소드를 생성 ({} 안의 부분은 반복 사용 가능)
...
//  delete ... By, remove ... By : delete 명령을 수행하는 키워드, delete ... By 는 리턴 값이 없음, remove ... By 는 삭제 수를 반환
...
//  주제 키워드와 By 사이에 입력
  List<ProductEntity> findFirst5ByName(String name); // 조회된 결과 중 처음의 5개 데이터만 출력
  List<ProductEntity> findTop5ByNumber(Long number); // 조회된 결과를 숫자가 큰 것 5개 데이터만 출력
...
//  (Is)Null, (Is)NotNull : 값이 null 인지 확인하는 조건 키워드, IS 생략 가능
  List<ProductEntity> findByUpdatedAtNull(); // 수정된 데이터가 null 인 것만 조회
  List<ProductEntity> findByUpdatedAtIsNull();
  List<ProductEntity> findByUpdatedAtNotNull(); // 수정된 데이터가 null 아닌 것만 조회
...
//  OrderBy ... Asc|Desc ... Asc|Desc : 쿼리문의 결과를 지정한 컬럼으로 정렬하는 키워드
  ...
  List<ProductEntity> findByNameOrderByPriceAscStockDesc(String name); // 지정한 이름으로 데이터 조회 후 가격을 기준으로 오른차순 정렬을 먼저하고, 재고 수량을 기준으로 내림차순 정렬

//  정렬 사용 시 Sort 객체를 사용하여 정렬하는 것이 가능함
//  쿼리 메소들 생성 후 매개변수에 Sort 객체를 추가하여 정렬
...
//  @Query : 복잡한 쿼리문은 쿼리 메소드 생성하기 힘들기 때문에 추가된 어노테이션
...
//  SELECT 절에 클래스의 별명.컬럼명으로 출력할 컬럼만 입력
//  전체 컬럼 출력 시 클래스의 별명만 입력 ( * 아님)
//  WHERE 절에는 클래스 별명.컬럼명 형태로 입력
  @Query("SELECT p FROM ProductEntity AS p WHERE p.name = '코카콜라'")
  ProductEntity querySelectName();

//  메소드의 매개변수를 쿼리문에 적용할 경우 '?순번' 형식으로 입력하여 데이터를 전달
//  순번은 1번부터 시작, 매개변수의 순서대로 입력 됨
  @Query("SELECT p FROM ProductEntity AS p WHERE p.name = ?1 AND p.price = ?2 ")
  ProductEntity querySelectNameAndPrice(String name, long price);

//  메소드의 매개변수를 쿼리문에 적용할 경우 ':매개변수명' 형식으로 입력ㅎ하여 데이터를 전달
//  ':매개변수명' 방식은 메소드의 매개변수 앞에 @Param("컬럼명") 어노테이션이 필요함
//  ':매개변수명' 방식은 매개변수의 순서와 상관없음
  @Query("SELECT p FROM ProductEntity AS p WHERE p.name = :name AND p.price = :price ")
  ProductEntity querySelectNameAndPrice(@Param("price") long price, @Param("name") String name);
}
